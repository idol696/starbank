...
class TelegramBotServiceTest {
    ...
    void shouldReturnNoRecommendationsForUser() {
        ...
        RecommendationResponse emptyResponse = new RecommendationResponse(id,List.of());
    ...
    void shouldReturnUserNotFoundMessage() {
        ...
        when(recommendationService.getRecommendationsByUserName(username)).thenThrow(new UserNotFoundException());

        String response = telegramBotService.getBotRecommendationByUsername(username);

        assertEquals("Пользователь не найден, проверьте правильность написания имени и фамилии", response);
    }
